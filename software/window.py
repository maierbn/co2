...
while True:
	...
	with open(command_filename, "r") as file:
		...
		if command == "open" and not is_open:
			motor.turn_open()
			is_open = True

		elif command == "open5" and not is_open:
			motor.turn_open()
			is_open = True
			forced_open = True
			forced_end_time = datetime.datetime.now() + datetime.timedelta(minutes=5)

		elif command == "close" and is_open:
			motor.turn_close()
			is_open = False

		elif command == "close5" and is_open:
			motor.turn_close()
			is_open = False
			forced_close = True
			forced_end_time = datetime.datetime.now() + datetime.timedelta(minutes=5)
	...
	if forced_open or forced_close:
		continue	 
